...
## Local Browser Curation

Use the local curator module to review a local dataset in a browser, play episodes, keep frame ranges, delete frame ranges, or delete whole episodes. The tool edits the local dataset in place after each operation, so copy the dataset first if you need a rollback.

For a dataset in the default Hugging Face cache:
...
If `--repo-id` is omitted, the tool uses the dataset folder name. Video-backed datasets stream videos directly in the browser; image-backed datasets use frame previews.
