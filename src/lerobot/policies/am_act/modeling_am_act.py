...
class AMACTPolicy(PreTrainedPolicy):
    """
    Action Chunking Transformer Policy as per Learning Fine-Grained Bimanual Manipulation with Low-Cost
    Hardware (paper: https://huggingface.co/papers/2304.13705, code: https://github.com/tonyzhaozh/act)
    """

    config_class = AMACTConfig
    name = "am_act"
    ...
    def __init__(
        ...
        **kwargs,
        ...
        invalid_dims.extend(dim for dim in config.discrete_action_dims if dim >= action_dim)
        if invalid_dims:
            raise ValueError(
                f"ACT action dimension is {action_dim}, but configured indices {invalid_dims} are invalid."
            )
        if config.robot_state_feature and config.observation_state_dims:
            state_dim = config.robot_state_feature.shape[0]
            invalid_state_dims = [dim for dim in config.observation_state_dims if dim >= state_dim]
            if invalid_state_dims:
                raise ValueError(
                    f"ACT observation.state dimension is {state_dim}, but configured indices "
                    f"{invalid_state_dims} are invalid."
                )
    ...
    def _initialize_discrete_action_values(self, dataset_stats: dict | None) -> None:
        if not self.config.discrete_action_dims:
            return
        # A saved checkpoint already contains centers for evaluation. During
        # fine-tuning, however, make_policy supplies the current dataset stats;
        # always recompute so decoding through the new unnormalizer still yields
        # the exact physical classes.
        if dataset_stats is None:
            if self.config.discrete_action_normalized_values:
                return
            raise ValueError(
                "ACT discrete action heads need dataset statistics when normalized values are absent."
            )
        if ACTION not in dataset_stats:
            raise ValueError("ACT dataset statistics are missing the action feature.")
        ...
        normalized_values = []
        for dim, physical_values in zip(
            self.config.discrete_action_dims,
            self.config.discrete_action_values,
            strict=True,
        ):
            if std[dim].abs() < 1e-8:
                raise ValueError(f"Action dimension {dim} has zero std and cannot be classified.")
            values = torch.as_tensor(physical_values, dtype=torch.float32)
            normalized_values.append(((values - mean[dim]) / std[dim]).tolist())
        self.config.discrete_action_normalized_values = normalized_values
        logging.info(
            "ACT normalized discrete action values: %s",
            self.config.discrete_action_normalized_values,
        )
    ...
    def forward(self, batch: dict[str, Tensor]) -> tuple[Tensor, dict]:
        ...
        if self.config.action_loss_groups:
            ...
            for name, dims in self.config.action_loss_groups.items():
                ...
                if not active_group_dims:
                    continue
                group_err = abs_err[..., active_group_dims]
                group_loss = (group_err * valid_mask).sum() / (
                    valid_mask.sum() * len(active_group_dims)
                ).clamp_min(1)
        ...
        discrete_losses = []
        for head_index, (dim, logits) in enumerate(
            zip(self.config.discrete_action_dims, discrete_logits, strict=True)
        ):
            centers = torch.as_tensor(
                self.config.discrete_action_normalized_values[head_index],
            ...
            if self.config.discrete_action_class_weights:
                class_weights = torch.as_tensor(
                    self.config.discrete_action_class_weights[head_index],
                    dtype=logits.dtype,
...
class ACT(nn.Module):
    """Action Chunking Transformer: The underlying neural network for AMACTPolicy.
    ...
    def __init__(self, config: AMACTConfig):
        ...
        if self.config.use_vae:
            ...
            if self.config.robot_state_feature:
                robot_state_dim = (
                    len(config.observation_state_dims)
        ...
        if self.config.robot_state_feature:
            ...
            self.encoder_robot_state_input_proj = nn.Linear(robot_state_dim, config.dim_model)
        ...
        self.discrete_action_heads = nn.ModuleList(
            nn.Linear(config.dim_model, len(values)) for values in config.discrete_action_values
        )
        ...
        *,
        ...
        if self.config.use_vae and ACTION in batch and self.training:
            ...
            if self.config.robot_state_feature:
                ...
                if self.config.observation_state_dims:
                    robot_state = robot_state[..., self.config.observation_state_dims]
        ...
        discrete_logits = [head(decoder_out) for head in self.discrete_action_heads]
        if discrete_logits:
            actions = actions.clone()
            for head_index, (dim, logits) in enumerate(
                zip(self.config.discrete_action_dims, discrete_logits, strict=True)
                ...
                actions[..., dim] = centers[logits.argmax(dim=-1)]
...
class ACTEncoder(nn.Module):
    ...
    def __init__(self, config: AMACTConfig, is_vae_encoder: bool = False):
        super().__init__()
